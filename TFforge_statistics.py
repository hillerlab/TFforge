...
def __get_arguments():
	...
	app.add_argument("--no_missing_sequences", action="store_true", help="Skip tree pruning step")
...
def compute_and_output_significance(args, scoreData, transcription_factors):
	...
	with open("significant_factors%s"%full_suffix, "w") as summary:
		...
		for tf in transcription_factors:
			if tf+"+" in scoreData and tf+"-" in scoreData:
				...
				scores = scoreData[tf+"+"] + scoreData[tf+"-"]
				if phenotypes.count(0) > 1 and phenotypes.count(1) > 1:
					try:
						pvalue = test_significance(scores, phenotypes)
					except Exception as err:
						logging.warning("Error in test_significance of %s"%tf)
						logging.warning("scores: %s"%(str(scores)))
						logging.warning("phenotypes: %s"%(str(phenotypes)))
						raise err
					summary.write("{0:15}\t{1}\t{2}\t{3}\n".format(tf, pvalue, phenotypes.count(0), phenotypes.count(1)))
				else:
					summary.write("{0:15}\tNA\t{1}\t{2}\n".format(tf, phenotypes.count(0), phenotypes.count(1)) )
			else:
				logging.info("scoreData for transcription factor %s is missing"%tf)
...
def __analyse_sequences():
	...
	if args.no_missing_sequences:
		tree = Phylo.read(args.treefile, 'newick')
		phylo = dollo_parsimony(tree, losses)						# annotate with trait
		sub_phylos = dict.fromkeys(elements, phylo)
	else:
		sub_phylos = {}
		for cne in elements:
	...
	scoreData = {key+'+':[] for key in transcription_factors}
	scoreData.update({key+'-':[] for key in transcription_factors})
		...
		for branch_end in branch_ends:
			...
			for tf in transcription_factors:
				if branch in scoreFileContent[(tf,cne)]:
					...
					if trait:	scoreData[tf + "+"].append(score)
					else: 		scoreData[tf + "-"].append(score)

	logging.debug("Hits: %d; Misses: %d"%(ch, cm))

	#########################
	# assess phenotype-divergence association
	compute_and_output_significance(args, scoreData, transcription_factors)
