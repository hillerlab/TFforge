...
def traverse_tree(parent, wtmx_file, scoring_window_length, sequences, background="", filter_threshold=None, 
	...
	try:
		...
		if sequence in sequence_score_cache.keys():
			...
			if fixProbs:
				if os.path.exists(str(fixProbs)):
					# use transition probabilities of the ancestral species
					...
					# compute transition probabilities for the ancestral species
		...
		for child in parent:
			# recursive traversal of the phylogenetic tree
		...
		try:
			# exists only if fixProbs is set and only for the ancestral species
